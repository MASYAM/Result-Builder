...
{   
    ...
    $term_name    = $db->find_by_sql("term","terms ORDER BY term_priority","","");
    ...
    if ($student !== 'No Result Found') 
    {
        ...
        {
            $exm_type[$subj_val['subject']] = $db->find_by_sql("DISTINCT exam_type,full_mark", "exam_type_list", " section='$section_name[0]' AND subject='{$subj_val['subject']}'  ORDER BY exam_type", "");
        ...
        $class_name = $db->find_by_sql("DISTINCT class_name","class_n_shift_name"," section='$section_name[0]'","");
        if($class_name !== 'No Result Found'){ $cn = $class_name[0]['class_name']; }else{ $cn = 'Not set'; }
        $file="Class:($cn)-Section:($section_name[0])-(Aggregate).xls";
       ...
       // $r .='<div><b>Term: ' . $term_name[0] . ', Section: ' . $section_name[0] . '</b></div>';
        ...
        {
            $count_exam_type = 0;
            foreach ($term_name as $term_value) {
                 $count_exam_type += 1;
            }
            $count_exam_type = $count_exam_type + 1;//for aggregate
            $r .='<td style="text-align:center;" colspan="'.$count_exam_type.'">' . $subj_value['subject'] . '</td>';
        }
        foreach ($term_name as $term_value) {
           $r .='<td style="text-align:center;" colspan="3"></td>';
        }
        $r .='<td></td>';
        $r .='</tr>';
        $r .='<tr style="background:PapayaWhip ;color:#000;font-weight:bold;">';
        ...
        foreach ($subject as $subj_value) 
        {
            foreach ($term_name as $term_value) {
                 $count_exam_type = 1;
                 $r .='<td style="text-align:center;" colspan="'.$count_exam_type.'">' . $term_value['term'] . '</td>';
            }
            $r .='<td style="text-align:center;" colspan="1">Aggregate</td>';
        }
        foreach ($term_name as $term_value) {
           $r .='<td style="text-align:center;" colspan="3">' . $term_value['term'] . '</td>';
        }
        $r .='<td></td>';
        $r .='</tr>';
        ...
        {                       
            ...
            foreach ($subject as $subj_value) 
            {
                foreach ($term_name as $term_value)
                {
                        $grade[$subj_value['subject']] = $db->find_by_sql("num_from,num_to,grade,point", "grade_list", "term='{$term_value['term']}' AND section='$section_name[0]' AND subject='{$subj_value['subject']}'", "");
                         ...
                         foreach ($exm_type[$subj_value['subject']] as $exam_type_value) 
                         {
                             ...
                             $number = $db->find_by_sql("number", "student_n_numbers", "term='{$term_value['term']}' AND section='$section_name[0]' AND student_id='{$stu_value['student_id']}' AND subject='{$subj_value['subject']}' AND exam_type='{$exam_type_value['exam_type']}'", "");
                             ...
                             //$r .=$number_value;
                             //$r .='</td>';
                             $number_value = ''; //set $number_value as an empty var
                         }
                          ...
                          $catch_addition = round($catch_addition); // make total number into round figure
                             ...
                             $aggregate_addition += $catch_addition;
                             $catch_addition = ''; //set this var as empty
                             $desire_grade = ''; //set this var as empty
                }//end loop of terms
                $r .='<td style="text-align:center;"><b>'.$aggregate_addition.'<b></td>';
               // $r .='<td style="text-align:center;"><b><b></td>';
                $aggregate_addition = 0;
            }
            $class_roll = '';
            foreach ($term_name as $term_value) {
                $disp_n_roll_result = $db->find_by_sql("working_days,absent,detention,class_roll","discipline_n_roll","term='{$term_value['term']}' AND section='$section_name[0]' AND student_id='{$stu_value['student_id']}'","") ;
                if($disp_n_roll_result !== 'No Result Found')
                { $working_days = $disp_n_roll_result[0]['working_days']; $absent = $disp_n_roll_result[0]['absent'];
                  $detention = $disp_n_roll_result[0]['detention'];  if(empty($class_roll) == FALSE){$class_roll = $disp_n_roll_result[0]['class_roll'];} 
                  
                }else{ $working_days = ''; $absent = ''; $detention = ''; }
